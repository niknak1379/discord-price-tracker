...
func GetLastAttemptByCrawler() ([]CrawlerProxyStats, error) {
	collection := Client.Database("tracker").Collection("Incidents")
	pipeline := mongo.Pipeline{
		bson.D{{Key: "$addFields", Value: bson.D{
			{Key: "lastAttempt", Value: bson.D{
				{Key: "$arrayElemAt", Value: bson.A{"$Attempts", -1}},
			}},
		}}},
		bson.D{{Key: "$group", Value: bson.D{
			{Key: "_id", Value: bson.D{
				{Key: "Crawler", Value: "$lastAttempt.Crawler"},
				{Key: "Proxy", Value: "$lastAttempt.Proxy"},
			}},
			{Key: "Count", Value: bson.D{{Key: "$sum", Value: 1}}},
		}}},
		bson.D{{Key: "$sort", Value: bson.D{
			{Key: "_id.Crawler", Value: 1},
			{Key: "_id.Proxy", Value: 1},
		}}},
	}
	...
	cursor, err := collection.Aggregate(ctx, pipeline)
	if err != nil {
		slog.Error("failed to aggregate last attempt by crawler", slog.Any("error", err))
		return nil, err
	}
	if err := cursor.All(ctx, &rawResults); err != nil {
		slog.Error("failed to decode last attempt by crawler", slog.Any("error", err))
		return nil, err
	}

	crawlerMap := make(map[string][]ProxyCount)
	for _, r := range rawResults {
		crawlerMap[r.Crawler] = append(crawlerMap[r.Crawler], ProxyCount{
			ProxyType: r.Proxy,
			Count:     r.Count,
		})
	}

	var results []CrawlerProxyStats
	for crawler, proxyStats := range crawlerMap {
		results = append(results, CrawlerProxyStats{
			Crawler:    crawler,
...
func GetProxyFailureRate() ([]ProxyStats, error) {
	collection := Client.Database("tracker").Collection("Incidents")
	pipeline := mongo.Pipeline{
		bson.D{{Key: "$addFields", Value: bson.D{
			{Key: "lastAttempt", Value: bson.D{
		...
		bson.D{{Key: "$group", Value: bson.D{
			{Key: "_id", Value: "$lastAttempt.Proxy"},
			{Key: "Total", Value: bson.D{{Key: "$sum", Value: 1}}},
			{Key: "Resolved", Value: bson.D{
				{Key: "$sum", Value: bson.D{
					{Key: "$cond", Value: bson.A{
						bson.D{{Key: "$eq", Value: bson.A{"$Resolved", true}}},
						1,
						0,
					}},
				}},
	...
	type rawProxyStats struct {
		ProxyType string `bson:"_id"`
		Total     int    `bson:"Total"`
		Resolved  int    `bson:"Resolved"`
	}

	var rawResults []rawProxyStats
	cursor, err := collection.Aggregate(ctx, pipeline)
	if err != nil {
		slog.Error("failed to aggregate proxy failure rate", slog.Any("error", err))
		return nil, err
	}
	if err := cursor.All(ctx, &rawResults); err != nil {
		slog.Error("failed to decode proxy failure rate", slog.Any("error", err))
		return nil, err
	}

	var results []ProxyStats
	for _, r := range rawResults {
		successRate := 0.0
		if r.Total > 0 {
			successRate = float64(r.Resolved) / float64(r.Total) * 100
		}
		results = append(results, ProxyStats{
			ProxyType:   r.ProxyType,
			Total:       r.Total,
			Failed:      r.Total - r.Resolved,
